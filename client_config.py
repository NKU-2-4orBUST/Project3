...
def config():      
  ...
  s('echo "Configuring rsyslog and Monit"; sed "s/IP_SERVER.*/'+args.server+'/g" monitrc_client > monitrc1; sed "s/IP_CLIENT.*/'+args.client+'/g" monitrc1 > /etc/monitrc; sed "s/IP_SERVER.*/'+args.server+':514/g" rsyslog.conf > /etc/rsyslog.conf; systemctl enable monit rsyslog; systemctl start monit rsyslog; chmod 0700 /etc/monitrc')
