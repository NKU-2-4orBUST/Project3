...
s('echo ""; echo "Testing SSH..."; systemctl stop sshd; systemctl status sshd; sleep 15; echo "verifying monit sees SSH back up..."; monit summary | grep SSH')
...
s('echo ""; echo "Testing SENDMAIL..."; systemctl stop sendmail; monit; sleep 1; monit summary | grep SENDMAIL; echo "Waiting 2 minutes for SENDMAIL services to restart"; sleep 120; monit summary | grep SENDMAIL')

# Verifiy Monit monitors and restarts LDAP
s('echo ""; echo "Testing LDAP..."; systemctl stop nslcd; monit; sleep 1; monit summary | grep LDAP; sleep 35; monit summary | grep LDAP')

# Verifiy Monit monitors Memory usages
s('echo ""; echo "Testing System Memory Monitoring..."; monit; sleep 1; monit summary | grep System; stress --vm  1 --vm-bytes 3500M --timeout 45s; monit summary | grep System')

# Verifiy Monit monitors CPU usages
s('echo ""; echo "Testing System CPU Monitoring..."; monit; sleep 1; monit summary | grep System; stress --vm-bytes 512M --cpu 100 --timeout 90s; monit summary | grep System')
